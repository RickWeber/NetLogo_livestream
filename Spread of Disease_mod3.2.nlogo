;;; add an ice cream store

turtles-own [
  infected?    ;; has the person been infected with the disease?
  hospitalized?
  recovered?   ;; can they catch/pass on the disease?
  cost         ;; private costs suffered by turtles
  recovery-time
  getting-icecream? ;; am I going to get some ice cream.
]
...
to go
  ...
  recolor
  if icecream? [
    let X count turtles with [ getting-icecream? ]
    if X < ice-cream-buyers [
      ask n-of (ice-cream-buyers - X) turtles with [ not hospitalized? ] [
        set getting-icecream? true
        ; set size 4 ; uncomment to see icecream seeking turtles for debugging purposes
      ]
    ]
    ask turtles with [ getting-icecream? ] [ get-icecream ]
  ]
  move
...
to get-icecream
  facexy 0 0
;  setxy 0 0 ; beam directly to the store
  if (patch-here = patch 0 0)[
    set getting-icecream? false
;    setxy random-xcor random-ycor ; alternate movement rule
  ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot cumulative-costs"
